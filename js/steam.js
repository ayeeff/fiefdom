(function() {
    const sites = [
        ...
        {name: "Tianjin THATIC", province: "Tianjin", city: "Tianjin", lat: 39.09, lon: 117.70, products: ["Server CPUs", "Embedded CPUs"], capacity: "Not Deck APU", notes: "AMD 51% JV, Entity List 2019"}
    ];
    ...
    sites.forEach((site, index) => {
        ...
        row.innerHTML = `<td style="font-weight: 500;">${site.name}</td><td><span class="steamsupply-color-dot" style="background-color: ${colors[index]}"></span></td><td>${site.province}</td><td>${site.products.join(', ')}</td><td style="font-size: 0.75rem;">${site.notes}</td>`;
